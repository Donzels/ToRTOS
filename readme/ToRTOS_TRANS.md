...
目标：将 ToRTOS 运行在新的 Cortex-M 或其它架构 MCU 上。核心点：上下文切换、栈初始化、关中断、SysTick 时基、最低限度的汇编支持函数、可选的位操作 (t_ffs/t_fls)。
...
| 分类 | 函数 | 说明 |
| ---- | ---- | ---- |
| 上下文切换 | `t_normal_switch_task(prev, next)` | 保存当前线程 PSP，恢复下一线程 PSP，触发 PendSV/直接切换 |
| 首次切换 | `t_first_switch_task(next)` | 初始进入第一个线程 |
| 栈初始化 | `t_stack_init(entry, stack_top, arg)` | 构造初始硬件+软件栈帧，LR 指向 `t_thread_exit` |
| 中断控制 | `t_irq_disable(void)` / `t_irq_enable(level)` | 使用 PRIMASK 或 BASEPRI |
| 位扫描 | `__t_ffs(value)` 或 `__t_fls(value)` | 查找最低/最高有效 1 bit（可用 CLZ 或内联循环） |
| SysTick 钩子 | 调用 `t_tick_increase()` | 在系统节拍中断中调用 |
...
### 2.2 上下文切换
典型做法：
1. 在 `t_normal_switch_task` 中存储当前 PSP 到 *prev
2. 载入 *next 到 PSP
...
- 在 `SysTick_Handler` 中调用t_tick_increase()函数 ：
```
...
| 问题 | 现象 | 解决 |
| ---- | ---- | ---- |
| 栈未对齐 | HardFault | 在 `t_stack_init` 处理 8 字节对齐 |
| 未正确保存 R4-R11 | 切换后变量错乱 | 在汇编保存/恢复低/高寄存器 |
| SysTick 未调用 `t_tick_increase` | 无时间片/睡眠失效 | ISR 添调用 |
| `__t_ffs/__t_fls` 返回错误 | 调度不工作或死循环 | 测试位图：输入 0x10 应返回 5 |
| 忘记设置 LR | 线程 return 后 HardFault | 栈帧 LR= `t_thread_exit` |
