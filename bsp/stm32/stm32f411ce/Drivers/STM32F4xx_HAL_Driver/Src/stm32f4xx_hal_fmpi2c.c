                                               ...
                                               uint16_t Size)
{
  ...
  if (hfmpi2c->State == HAL_FMPI2C_STATE_READY)
  {
    ...
    /* Send Slave Address */
    /* Set NBYTES to write and reload if hfmpi2c->XferCount > MAX_NBYTE_SIZE */
    FMPI2C_TransferConfig(hfmpi2c, DevAddress, (uint8_t)hfmpi2c->XferSize, xfermode, FMPI2C_GENERATE_TO_READ);
    ...
    if (hfmpi2c->XferSize > 0U)
    {
      ...
      if (dmaxferstatus == HAL_OK)
      {
        ...
        FMPI2C_TransferConfig(hfmpi2c, DevAddress, (uint8_t)(hfmpi2c->XferSize + 1U),
                              xfermode, FMPI2C_GENERATE_TO_WRITE);
                                          ...
                                          uint16_t MemAddSize, uint8_t *pData, uint16_t Size)
{
  ...
  if (hfmpi2c->State == HAL_FMPI2C_STATE_READY)
  {
    ...
    /* Send Slave Address and Memory Address */
    FMPI2C_TransferConfig(hfmpi2c, DevAddress, (uint8_t)MemAddSize, FMPI2C_RELOAD_MODE, FMPI2C_GENERATE_TO_WRITE);
                                         ...
                                         uint16_t MemAddSize, uint8_t *pData, uint16_t Size)
{
  ...
  if (hfmpi2c->State == HAL_FMPI2C_STATE_READY)
  {
    ...
    /* Send Slave Address and Memory Address */
    FMPI2C_TransferConfig(hfmpi2c, DevAddress, (uint8_t)MemAddSize, FMPI2C_SOFTEND_MODE, FMPI2C_GENERATE_TO_WRITE);
...
{
  uint32_t xfermode;
  uint32_t xferrequest = FMPI2C_GENERATE_TO_WRITE;
  uint32_t sizetoxfer = 0U;
...
{
  uint32_t xfermode;
  uint32_t xferrequest = FMPI2C_GENERATE_TO_READ;
...
static HAL_StatusTypeDef FMPI2C_Mem_ISR_IT(struct __FMPI2C_HandleTypeDef *hfmpi2c, uint32_t ITFlags,
                                           uint32_t ITSources)
{
  uint32_t direction = FMPI2C_GENERATE_TO_WRITE;
  uint32_t tmpITFlags = ITFlags;
           ...
           (FMPI2C_CHECK_IT_SOURCE(ITSources, FMPI2C_IT_TCI) != RESET))
  {
    ...
    {
      direction = FMPI2C_GENERATE_TO_READ;
    }
...
/**
  ...
  *     @arg @ref FMPI2C_GENERATE_TO_READ Generate Restart for read request.
  *     @arg @ref FMPI2C_GENERATE_TO_WRITE Generate Restart for write request.
  * @retval None
  */
static void FMPI2C_TransferConfig(FMPI2C_HandleTypeDef *hfmpi2c, uint16_t DevAddress, uint8_t Size, uint32_t Mode,
                                  uint32_t Request)
